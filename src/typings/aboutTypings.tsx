import { GithubSvg } from "../images/githubSvg";
import { GlassSvg } from "../images/glassSvg";
import { LinkedinSvg } from "../images/linkedinSvg";
import { SocialCardInterface } from "../interfaces/interface";

export const socialCardsTypings: SocialCardInterface[] = [
  {
    imgUrl: "https://avatars.githubusercontent.com/u/104111332?v=4",
    name: "Guilherme Dib da Silva",
    description:
      "Desenvolvedor FullStack estudante de ciencia da computacao (4 semestre) na universidade catolica de santos",
    socials: [
      {
        name: "Github",
        nameAt: "gui23dib",
        icon: <GithubSvg />,
        link: "https://github.com/gui23dib",
      },
      {
        name: "LinkedIn",
        nameAt: "Guilherme Dib da Silva",
        icon: <LinkedinSvg />,
        link: "https://www.linkedin.com/in/guilherme-dib-da-silva-50288523b/",
      },
      {
        name: "Curriculum Lattes",
        nameAt: "3047829595983824",
        icon: <GlassSvg />,
        link: "http://lattes.cnpq.br/3047829595983824",
      },
    ],
  },
  {
    imgUrl: "https://avatars.githubusercontent.com/u/103036731?v=4",
    name: "Kadish Bernardo Ribeiro da Silva",
    description: "xxx",
    socials: [
      {
        name: "Github",
        nameAt: "Kadish-Bernardo",
        icon: <GithubSvg />,
        link: "https://github.com/Kadish-Bernardo",
      },
      {
        name: "LinkedIn",
        nameAt: "Kadish Silva",
        icon: <LinkedinSvg />,
        link: "https://www.linkedin.com/in/kadish-bernardo/",
      },
      {
        name: "Curriculum Lattes",
        nameAt: "",
        icon: <GlassSvg />,
        link: "",
      },
    ],
  },
  {
    imgUrl: "https://avatars.githubusercontent.com/u/114711381?v=4",
    name: "Larissa Sena Peirera",
    description: "xxx",
    socials: [
      {
        name: "Github",
        nameAt: "Larissaseninha",
        icon: <GithubSvg />,
        link: "https://github.com/Larissaseninha",
      },
      {
        name: "LinkedIn",
        nameAt: "Larissa Sena",
        icon: <LinkedinSvg />,
        link: "https://www.linkedin.com/in/larissa-sena-a5b065192/",
      },
      {
        name: "Curriculum Lattes",
        nameAt: "",
        icon: <GlassSvg />,
        link: "",
      },
    ],
  },
  {
    imgUrl: "https://avatars.githubusercontent.com/u/99672109?v=4",
    name: "Murilo Mendes Santo Sardão",
    description: "xxx",
    socials: [
      {
        name: "Github",
        nameAt: "Sardao04",
        icon: <GithubSvg />,
        link: "https://github.com/Yruc4",
      },
      {
        name: "LinkedIn",
        nameAt: "Murilo Mendes Santo Sardão",
        icon: <LinkedinSvg />,
        link: "https://www.linkedin.com/in/murilo-mendes-santo-sardão-b9a532239/",
      },
      {
        name: "Curriculum Lattes",
        nameAt: "",
        icon: <GlassSvg />,
        link: "",
      },
    ],
  },
  {
    imgUrl: "https://avatars.githubusercontent.com/u/103077237?v=4",
    name: "Thiago de Andrade Prado",
    description: "xxx",
    socials: [
      {
        name: "Github",
        nameAt: "ThiagoAPrado",
        icon: <GithubSvg />,
        link: "https://github.com/ThiagoAPrado",
      },
      {
        name: "LinkedIn",
        nameAt: "Thiago de Andrade Prado",
        icon: <LinkedinSvg />,
        link: "https://www.linkedin.com/in/thiagoaprado",
      },
      {
        name: "Curriculum Lattes",
        nameAt: "9785326840091327",
        icon: <GlassSvg />,
        link: "http://lattes.cnpq.br/9785326840091327",
      },
    ],
  },
  {
    imgUrl: "https://avatars.githubusercontent.com/u/117671474?v=4",
    name: "Victor Eduardo Rodrigues de Abreu e Silva",
    description: "xxx",
    socials: [
      {
        name: "Github",
        nameAt: "VicEduDev",
        icon: <GithubSvg />,
        link: "https://github.com/vicEduDev",
      },
      {
        name: "LinkedIn",
        nameAt: "Victor Eduardo Rodrigues de Abreu e Silva",
        icon: <LinkedinSvg />,
        link: "https://www.linkedin.com/in/victor-eduardo-rodrigues-de-abreu-e-silva-b9abb3227/",
      },
      {
        name: "Curriculum Lattes",
        nameAt: "6930521779437025",
        icon: <GlassSvg />,
        link: "http://lattes.cnpq.br/6930521779437025",
      },
    ],
  },
];
